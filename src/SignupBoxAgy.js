...
export default function SignupBoxAgy() {

    const navigate = useNavigate();

    const context = useContext(dataContext);
    const { showAlert, setlogin, setDisplay, setProfile } = context;


    const [credentials, setCredentials] = useState({ email: "", aname: "", password: "", cpassword: "" })
    ...
    async function handleSubmit(e) {
        ...
        if (credentials.password === credentials.cpassword) {
            ...
            const response = await fetch(`https://car-rental.adaptable.app/api/authagency/createuser`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'

                },
                body: JSON.stringify({ agencyname: credentials.aname, email: credentials.email, password: credentials.password })
        ...
        <div className="container_login">
            ...
            <form className="myform" onSubmit={handleSubmit}>
                ...
                <i className="fa-solid fa-user px-1"></i>
                    <label htmlFor="exampleInputPassword1" className="form-label login_form">Agency Name</label>
                    <input type="name" className="form-control" id="aname" name='aname'
                        placeholder="Enter your Agency name" value={credentials.aname} onChange={onChange} />
